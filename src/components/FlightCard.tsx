...
export default function FlightCard({ flight }: FlightCardProps) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden transform hover:-translate-y-1 group">
      <div className="p-6">
        {/* Flight Header */}
        <div className="flex justify-between items-start mb-4">
          <div className="flex items-center space-x-3">
            <div className="bg-blue-100 p-2 rounded-lg group-hover:bg-blue-200 transition-colors duration-300">
              <Plane className="h-6 w-6 text-blue-600 group-hover:text-blue-700 transition-colors duration-300" />
            </div>
            <div>
              <h3 className="font-bold text-lg text-gray-900 group-hover:text-blue-900 transition-colors duration-300">{flight.flightNumber}</h3>
              <p className="text-gray-600 group-hover:text-gray-700 transition-colors duration-300">{flight.airline}</p>
            </div>
          </div>
          <div className="text-right">
            <p className="text-sm text-gray-500 mb-1">Starting from</p>
            <p className="text-2xl font-bold text-blue-600 group-hover:text-blue-700 transition-colors duration-300">
              {formatPrice(getLowestPrice())}
            </p>
            <p className="text-sm text-gray-600 group-hover:text-gray-700 transition-colors duration-300">per person</p>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="text-center">
            <p className="text-2xl font-bold text-gray-900 group-hover:text-blue-900 transition-colors duration-300">{formatTime(flight.departureTime)}</p>
            <p className="text-sm text-gray-600 group-hover:text-gray-700 transition-colors duration-300">{flight.origin.code}</p>
            <p className="text-xs text-gray-500 group-hover:text-gray-600 transition-colors duration-300">{formatDate(flight.departureTime)}</p>
          </div>
          
          <div className="flex-1 mx-4">
            <div className="flex items-center justify-center space-x-2">
              <div className="h-px bg-gray-300 flex-1 group-hover:bg-blue-300 transition-colors duration-300"></div>
              <div className="text-center">
                <Clock className="h-4 w-4 text-gray-400 mx-auto mb-1 group-hover:text-blue-500 transition-colors duration-300" />
                <p className="text-xs text-gray-600 group-hover:text-gray-700 transition-colors duration-300">{flight.duration}</p>
                {flight.stops === 0 ? (
                  <p className="text-xs text-green-600 font-medium group-hover:text-green-700 transition-colors duration-300">Non-stop</p>
                ) : (
                  <p className="text-xs text-orange-600 group-hover:text-orange-700 transition-colors duration-300">{flight.stops} stop{flight.stops > 1 ? 's' : ''}</p>
                )}
              </div>
              <div className="h-px bg-gray-300 flex-1 group-hover:bg-blue-300 transition-colors duration-300"></div>
            </div>
          ...
          <div className="text-center">
            <p className="text-2xl font-bold text-gray-900 group-hover:text-blue-900 transition-colors duration-300">{formatTime(flight.arrivalTime)}</p>
            <p className="text-sm text-gray-600 group-hover:text-gray-700 transition-colors duration-300">{flight.destination.code}</p>
            <p className="text-xs text-gray-500 group-hover:text-gray-600 transition-colors duration-300">{formatDate(flight.arrivalTime)}</p>
          </div>
        </div>

        {/* Interactive Class Selection */}
        <div className="mb-4 p-4 bg-gray-50 rounded-lg group-hover:bg-blue-50 transition-colors duration-300">
          <h4 className="text-sm font-semibold text-gray-700 mb-3 group-hover:text-gray-800 transition-colors duration-300">
        ...
        {/* Flight Details */}
        <div className="flex items-center justify-between mb-4 p-3 bg-gray-50 rounded-lg group-hover:bg-blue-50 transition-colors duration-300">
          <div className="flex items-center space-x-4 text-sm text-gray-600 group-hover:text-gray-700 transition-colors duration-300">
            <div className="flex items-center space-x-1">
              <MapPin className="h-4 w-4 group-hover:text-blue-600 transition-colors duration-300" />
              <span>{flight.aircraft}</span>
            </div>
            <div className="flex items-center space-x-2">
              <Wifi className="h-4 w-4 text-blue-500 hover:text-blue-600 transition-colors duration-300" />
              <Coffee className="h-4 w-4 text-amber-600 hover:text-amber-700 transition-colors duration-300" />
              <Tv className="h-4 w-4 text-purple-500 hover:text-purple-600 transition-colors duration-300" />
            </div>
          </div>
